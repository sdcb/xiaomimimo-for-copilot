...
import {
	...
	createVSCodeLanguageModelVisionDescriberGetter,
...
import { testVisionProxyConnection, type VisionProxyTestResult } from '../test';
...
async function handleMessage(
	...
	if (message.type === 'logVisionProxyTestFailure') {
		...
		logger.error('Vision proxy test failed:', errorMessage);
		return;
	}
	...
	try {
		...
		if (message.type === 'saveConfig') {
			...
			if (payload.source === 'vscode-lm') {
				await saveVSCodeVisionModelId(
					getRequiredString(payload.lmModelId, 'Vision Model'),
				);
		...
		if (message.type === 'testConnection') {
			const payload = getWebviewPayload(message.value);
			if (payload.source === 'vscode-lm') {
				postStatus(panel, 'VS Code language models are tested automatically when used.', 'info', undefined, {
					kind: 'test',
			...
			if (result.ok) {
				postStatus(panel, 'Connection test succeeded!', 'success', undefined, {
					kind: 'test',
					testId: payload.testId,
...
function createSavedMessage(payload: WebviewPayload): string {
	if (payload.source === 'vscode-lm') {
		return 'VS Code vision model saved.';
	}
	return payload.apiKey
		? 'API endpoint configuration and key saved.'
		: 'API endpoint configuration saved.';
}

function getVisionProxyTestFailure(result: VisionProxyTestResult): string {
	return formatVisionProxyDisplayMessage(
		result.errorCode ?? 'UNKNOWN',
		result.message ?? 'Vision proxy test failed.',
	);
}
...
function createShowLogsAction(): StatusAction {
	return { command: 'showLogs', label: 'View Details' };
}
...
function getWebviewPayload(value: unknown): WebviewPayload {
	...
	const source = payload.source === 'vscode-lm' || payload.source === 'api-endpoint'
		? payload.source
		: 'api-endpoint';
