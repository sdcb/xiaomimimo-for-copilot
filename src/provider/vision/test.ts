...
// Small captcha-style RGBA PNG with a 4-character code.
...
export async function testVisionProxyConnection(
	...
	try {
		...
		logger.info('Vision proxy test succeeded:', {
		...
		return {
			ok: false,
			errorCode: getVisionProxyErrorDisplayCode(error),
			message: error instanceof Error ? error.message : 'Vision proxy test failed.',
		};
	} finally {
		tokenSource.dispose();
	}
